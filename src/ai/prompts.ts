...
**x402** — make a micropayment-protected HTTP call (x402 protocol, auto-handles 402 challenge/pay/retry):
\`{ "id": string, "name": string, "type": "x402", "url": string, "method": "GET" | "POST", "bodyTemplate"?: string, "walletKeyEnvVar": "AGENT_WALLET_PRIVATE_KEY", "network": "base-sepolia", "maxAmountUsd": number }\`
Use this when the target API requires a USDC micropayment ($0.01 typical). Output: \`{ statusCode, body, paymentRequired, amountPaid, settlement }\`.
...
| Endpoint requires a USDC micropayment (x402 protocol) | \`x402\` |
...
### Example 6 — "HTTP webhook: pay $0.01 USDC via x402 to create a BTC price alert, then log the result"

{
  "irVersion": "1.0",
  "metadata": { "name": "x402-price-alert", "description": "HTTP trigger pays x402 to create a BTC alert, then extracts the alert id" },
  "runtime": { "defaultTarget": "local-simulation", "targets": { "local-simulation": { "rpcs": [{ "chainName": "ethereum-testnet-sepolia", "url": "https://ethereum-sepolia-rpc.publicnode.com" }], "broadcast": false, "receiverContract": "0x0000000000000000000000000000000000000000", "chainExplorerTxBaseUrl": "https://sepolia.etherscan.io/tx/" } } },
  "triggers": [{ "id": "trigger_1", "name": "Webhook", "type": "http", "authMode": "none" }],
  "actions": [
    {
      "id": "action_1",
      "name": "Create Alert (x402 Payment)",
      "type": "x402",
      "url": "http://localhost:3000/alerts",
      "method": "POST",
      "bodyTemplate": "{\"asset\":\"BTC\",\"condition\":\"gt\",\"targetPriceUsd\":60000}",
    ...
    },
    {
      ...
      "type": "transform",
      "template": {
        "alertId": "$outputs.action_1.body.alert.id",
        "amountPaid": "$outputs.action_1.amountPaid",
